...
function icsDate(d: Date) {
  // 2025-09-21T12:34:56.000Z -> 20250921T123456Z
...
=======
  ...
  // Load events once, derive available types from data
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        ...
        const types = Array.from(new Set(events.map((e) => e.type).filter(Boolean) as string[])).sort();
  ...
  // Filter + sort list (shared with map logic)
  ...
  // Recommended: normally from filteredSorted; if none, sample from all
...
=======
          ...
          <div className="rounded-2xl bg-white text-[#1b2432] p-4 overflow-hidden">
            ...
            <div className="relative [transform:none]">
              <Map dataUrl="/data/fallback-events.json" filters={{ types: selectedTypes, beginnerOnly, query, sortKey }} />
            </div>
          </div>
              ...
              recommended.map((e) => (
                <div key={e.id} className="rounded-xl bg-white text-[#1b2432] p-4 shadow-md">
                  <div className="flex items-start gap-3">
                    ...
                    {/* Removed the old standalone calendar icon box */}
                  </div>
                </div>
              ))
            )}
